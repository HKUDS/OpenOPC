...
def _apply_default_corporate_org(config: OPCConfig) -> None:
    """Populate the built-in Corporate org (roles + escalation rules) on a fresh
    config. Extracted verbatim from ``init`` — behaviour-preserving."""
...
def init(
    ...
    if already_initialized:
        ...
        if not config.org.roles:
            _apply_default_corporate_org(config)
        config.save(opc_home / "config")
