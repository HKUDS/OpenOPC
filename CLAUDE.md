...
- **Employee-loop incentive** (`docs/employee-loop-incentive.md`, skill `employee_loop_incentive` mounted on `founding_ai_native_lead` only, `contracts/VerifiedClosePayout.sol`): the CNO-designs-a-game doctrine for bottom-up loop-closing rewarded on-chain. The `.sol` is an explicitly **unaudited reference sketch** (no solc build in CI); `tests/test_employee_loop_incentive.py` guards its three invariants by text presence (maker≠checker, compounding vesting, safety-zero), not by compiling it.
